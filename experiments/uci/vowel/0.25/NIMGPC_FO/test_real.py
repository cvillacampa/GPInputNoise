...
Y = np.load("../../data/Y.npy").astype(np.int)

indices = np.load("../../data/splits.npy")[ fold, : ]
noise = np.load("../../data/noise25.npy")[ fold, : ]
...
model = NIMGPC_FO(kernels, inducing_points, n_classes, X_train, 0.1)

np.random.seed(0)

model.fit(X_train, y_train, tf.train.AdamOptimizer(learning_rate = 0.001), 1000, 50, X_test, y_test, "output_results_" + str(fold) + ".txt")
