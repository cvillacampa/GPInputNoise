...
import sys
sys.path.append("../../..")
from gp_input_noise.nimgpc_fo import NIMGPC_FO
...
model = NIMGPC_FO(kernels, inducing_points, n_classes, X_train)

np.random.seed(0)

model.fit(X_train, y_train, noise_var_train, tf.train.AdamOptimizer(learning_rate = 0.001), 750, 50, X_test, y_test, noise_var_test, "output_results_" + str(fold) + ".txt")
