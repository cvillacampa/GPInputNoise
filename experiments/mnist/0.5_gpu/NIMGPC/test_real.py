from mgpc_vi import MGPC_VI
from rbf_ard import RBF_ARD
import numpy as np
...
noise_var_train = np.zeros_like(X_train) + 0.5
noise_var_test = np.zeros_like(X_test) + 0.5
...
model = MGPC_VI(kernels, inducing_points, n_classes, X_train, 0.5)

np.random.seed(0)

model.fit(X_train, y_train, noise_var_train, tf.train.AdamOptimizer(learning_rate = 0.001), 350, 200, X_test, y_test, noise_var_test, "output_results_" + str(fold) + ".txt")

#result = model.predict(X_train, noise_var_train, 200)
#train_error = np.mean(result[ 1 ] != y_train[ : , 0 ]) 
#train_nll = -np.mean(np.log(result[ 0 ][ (np.arange(X_train.shape[ 0 ]), y_train[ :, 0 ]) ]))
#
#result = model.predict(X_test, noise_var_test, 200)
