...
class MGPC_VI(object):
    ...
    def __init__(self, kernels, inducing_points, n_classes, total_observed_x, input_var_ini, eps = 0.001):
        ...
        self.original_input_means, self.original_input_log_vars = neural_network(tf.concat([ self.noisy_train_inputs, \
            tf.cast(tf.one_hot(tf.reshape(self.train_outputs, (-1,)), self.n_classes, 1., 0.), tf.float32) ], 1), self.input_dims + self.n_classes, self.input_dims, 250, 2)
    ...
    def fit(self, training_points, training_targets, train_input_vars, optimizer, epochs, batch_size, test_points, test_targets, test_input_vars, file_results):
        ...
        ini_time = self.write_results(test_points, test_targets, test_input_vars, file_results, ini_time, 200)
