...
import sys
sys.path.append("../../../..")
from gp_input_noise.nimgpc_fo_learn_noise import NIMGPC_FO
...
X_train = X_train + noise[ 0 : n_train,  : ] * 0.0
X_test = X_test + noise[ n_train : (n_train + n_test),  : ] * 0.0
X_val = X_val + noise[ (n_train + n_test) : (n_train + n_test + n_val),  : ] * 0.0
...
model = NIMGPC_FO(kernels, inducing_points, n_classes, X_train, noise_var_ini)
...
for i in range(n_to_add):
        ...
        model.fit(X_train, y_train, optimizer, 1000, 50, X_test, y_test,  mode + "_output_results_" + str(fold) + ".txt", continue_training=True)
