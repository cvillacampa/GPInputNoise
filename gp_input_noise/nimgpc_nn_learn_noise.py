...
def neural_network(noisy_inputs, input_dims, output_dims, n_units, n_layers, soft_ini):

    W1_m = weight_variable([ input_dims, n_units]) 
    b1_m = bias_variable([ n_units ])

    W2_m = weight_variable([ n_units, n_units ])
    b2_m = bias_variable([ n_units ])

    W3_m = weight_variable([ n_units, output_dims ])
    b3_m = bias_variable([ output_dims ])

    W1_v = weight_variable([ input_dims, n_units]) 
    b1_v = bias_variable([ n_units ])

    W2_v = weight_variable([ n_units, n_units ])
    b2_v = bias_variable([ n_units ])

    W3_v = weight_variable([ n_units, output_dims ])

    if soft_ini:
        b3_v = tf.Variable(tf.truncated_normal([ output_dims ], stddev = 0.001) - 10.0)
    else:
        b3_v = bias_variable([ output_dims ])

    if n_layers == 2:
        h1_m = activation(tf.matmul(noisy_inputs, W1_m) + b1_m) 
        h2_m = activation(tf.matmul(h1_m, W2_m) + b2_m)
        mean = tf.matmul(h2_m, W3_m) + b3_m

        h1_v = activation(tf.matmul(noisy_inputs, W1_v) + b1_v) 
        h2_v = activation(tf.matmul(h1_v, W2_v) + b2_v)
        log_var = tf.matmul(h2_v, W3_v) + b3_v
    else:
        h1_m = activation(tf.matmul(noisy_inputs, W1_m) + b1_m) 
        mean = tf.matmul(h1_m, W3_m) + b3_m

        h1_v = activation(tf.matmul(noisy_inputs, W1_v) + b1_v) 
        log_var = tf.matmul(h1_v, W3_v) + b3_v
...
class NIMGPC_NN(object):

    '''
    Variational Inference multiclass classification with gaussian processes
    '''
    
    def __init__(self, kernels, inducing_points, n_classes, total_observed_x, input_var_ini, eps = 0.001, n_layers = 1, n_hiddens = 50, soft_ini = False):
        ...
        @param n_layers: Number of layers for the NN
        @param n_hiddens: Number of hidden units in the NN
        '''
        ...
        self.input_dims = total_observed_x.shape[ 1 ]
        ...
        self.n_samples_test = 500

        # We create here the parameters for the posterior distribution of the latent inputlocations
        ...
        self.train_indices = tf.placeholder(tf.float32, shape =  [None] , name = "train_indices")
        self.train_indices_input = tf.reshape(self.train_indices, [tf.shape(self.train_indices)[ 0 ], 1])

        self.original_input_means, self.original_input_log_vars = neural_network(tf.concat([ self.noisy_train_inputs, \
            tf.cast(tf.one_hot(tf.reshape(self.train_outputs, (-1,)), self.n_classes, 1., 0.), tf.float32) ], 1), self.input_dims + self.n_classes, self.input_dims, n_hiddens, n_layers, soft_ini)

        self.train_inputs = self.original_input_means + tf.sqrt(tf.exp(self.original_input_log_vars)) * tf.random_normal(tf.shape(self.noisy_train_inputs))

        self.posterior_test_variances = 1.0 / (1.0 / self.test_inputs_variances + 1.0 / self.variance_prior_over_data)
        self.posterior_test_means = (self.noisy_test_inputs * 1.0 / self.test_inputs_variances) * self.posterior_test_variances

        self.test_inputs =  tf.reshape(self.posterior_test_means[None,:,:] + tf.sqrt(self.posterior_test_variances[None,:,:]) * \
            tf.random_normal([ self.n_samples_test, tf.shape(self.noisy_test_inputs)[ 0 ], self.input_dim]), [ self.n_samples_test * tf.shape(self.noisy_test_inputs)[ 0 ], self.input_dim ])
    ...
    def _build_graph(self):
        ...
        expected_log_joint = self._build_expected_log_joint(self.train_inputs, self.train_outputs)
        expected_log_joint_test = self._build_expected_log_joint(self.test_inputs, self.test_outputs)
        expected_log_joint_noisy_train_data = self._build_expected_log_joint_noisy_data()
        KL_sum_noisy_data = self._build_KL_noisy_data()

        n_train = tf.shape(self.train_inputs)[ 0 ]

        nelbo = -1.0 * (tf.reduce_sum(expected_log_joint) * (self.total_training_data / tf.cast(n_train, tf.float32)) + \
            tf.reduce_sum(expected_log_joint_noisy_train_data) * (self.total_training_data / tf.cast(n_train, tf.float32)) \
            - KL_sum - KL_sum_noisy_data * (self.total_training_data / tf.cast(n_train, tf.float32))) 

        test_probs = self._build_prediction_probs()

        return [ nelbo, test_probs ]
 
    def _build_KL_noisy_data(self):
        return tf.reduce_sum(self.original_input_means**2 / (2.0 * self.variance_prior_over_data) + \
                             0.5 * (tf.exp(self.original_input_log_vars) / self.variance_prior_over_data - 1.0 - self.original_input_log_vars + tf.log(self.variance_prior_over_data))) 
    ...
    def _compute_probs_gh(self, data_inputs, data_targets):
    ...
    def _build_expected_log_joint_noisy_data(self):

        return tf.reduce_sum(- 0.5 * np.log(2 * np.pi) - 0.5 * tf.log(self.train_inputs_variances) - 0.5 * 1.0 / self.train_inputs_variances * \
            ((tf.square(self.original_input_means) + tf.exp(self.original_input_log_vars)) - 2.0 * self.noisy_train_inputs * self.original_input_means + self.noisy_train_inputs**2), axis = 1)

    def _build_expected_log_joint(self, data_inputs, data_outputs):

        probs = self._compute_probs_gh(data_inputs, data_outputs)
    ...
    def _build_prediction_probs(self):
        ...
        probs = self._compute_probs_gh(self.test_inputs, data_targets)

        probs_final = (1.0 - self.eps) * probs + (1.0 - probs) * self.eps / (self.n_classes - 1)

        return tf.reduce_mean(tf.reshape(probs_final, [ self.n_samples_test, tf.shape(self.noisy_test_inputs)[ 0 ] ]), axis = 0)


    def fit(self, training_points, training_targets, optimizer, epochs, batch_size, test_points, test_targets, file_results, continue_training = False):
        ...
        for e in range(epochs):
            ...
            for i in range(n_batches):
                ...
                avg_elbo += -1.0 * self.session.run(self.nelbo, feed_dict = {self.noisy_train_inputs: batch_x, self.train_outputs: batch_y})

                self.session.run(self.train_step, feed_dict = {self.noisy_train_inputs: batch_x, self.train_outputs: batch_y})
        ...
        ini_time = self.write_results(test_points, test_targets, file_results, ini_time, test_targets.shape[ 0 ] / 10)

    def write_results(self, data_points, data_targets, file_name, ini_time, batch_size = None):

        start_predict = time.time()
        result = self.predict(data_points, batch_size)
            ...
            for c in range(self.n_classes):
                ...
                probs[ i * batch_size : np.minimum((i + 1) * batch_size, test_inputs.shape[ 0 ]), c : (c + 1) ] = \
                    self.session.run(self.test_probs, feed_dict = {self.noisy_test_inputs: test_inputs[ indices , : ], \
		                             self.class_predict_test: np.array([ c ])})[:,None]
