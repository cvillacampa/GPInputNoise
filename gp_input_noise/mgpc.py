...
class MGPC(object):
    ...
    def __init__(self, kernels, inducing_points, n_classes, total_training_data, eps = 0.001):
        ...
        [ self.nelbo, self.test_probs, self.lower_bound_test ] = self._build_graph()
    ...
    def _build_graph(self):
        ...
        return [ nelbo, test_probs, expected_log_joint_test ]
    ...
    def _build_expected_log_joint(self, data_inputs, data_outputs):
        ...
        ret = tf.log(1.0 - self.eps + self.eps / self.n_classes) * probs + (1.0 - probs) * tf.log(self.eps / self.n_classes)
    ...
    def _build_prediction_probs(self):
        ...
        probs_final = (1.0 - self.eps) * probs + self.eps / self.n_classes
    ...
    def fit(self, training_points, training_targets, optimizer, epochs, batch_size, test_points, test_targets, file_results, continue_training=False):
        ...
        for e in range(epochs):
            ...
            for i in range(n_batches):
                ...
                batch_x = training_points[ i * self.batch_size : np.minimum((i + 1) * self.batch_size, \
                    training_points.shape[ 0 ]), : ].astype(np.float32)
                batch_y = training_targets[ i * self.batch_size : np.minimum((i + 1) * self.batch_size, \
                    training_points.shape[ 0 ]), : ].astype(np.int32)
            
                avg_elbo += -1.0 * self.session.run(self.nelbo, feed_dict = \
                    {self.train_inputs: batch_x, self.train_outputs: batch_y})

                self.session.run(self.train_step, feed_dict = \
                    {self.train_inputs: batch_x, self.train_outputs: batch_y})

            ini_time = self.write_results(test_points, test_targets, file_results, ini_time, test_targets.shape[ 0 ])
    ...
    def write_results(self, data_points, data_targets, file_name, ini_time, batch_size = None):
        ...
        exp_log_probs = np.mean(self.exp_log_probs(data_points, data_targets, batch_size))
        ...
        with open("results/" + file_name, "a") as myfile:
            myfile.write(str(error) + " " + str(nll) + " " + str(exp_log_probs) + " " + str(time.time() - ini_time) + '\n')
        ...
        for i in range(n_batches):
            for c in range(self.n_classes):
                probs[ i * batch_size : np.minimum((i + 1) * batch_size, test_inputs.shape[ 0 ]), c : (c + 1) ] = \
                    self.session.run(self.test_probs, feed_dict = {self.test_inputs: test_inputs[ i * \
                    batch_size : np.minimum((i + 1) * batch_size, test_inputs.shape[ 0 ]), : ], \
                    self.class_predict_test: np.array([ c ])})

        return [ probs, np.argmax(probs, 1) ] 

    def exp_log_probs(self, test_inputs, test_outputs, batch_size = None):
        ...
        values = np.zeros(test_inputs.shape[ 0 ])

        n_batches = int(np.ceil(float(test_inputs.shape[ 0 ]) / batch_size))
        
        for i in range(n_batches):
            values[ i * batch_size : np.minimum((i + 1) * batch_size, test_inputs.shape[ 0 ]) ] = \
                    self.session.run(self.lower_bound_test, feed_dict = {self.test_inputs: test_inputs[ i * \
                    batch_size : np.minimum((i + 1) * batch_size, test_inputs.shape[ 0 ]), : ], \
                    self.test_outputs: test_outputs[ i * batch_size : np.minimum((i + 1) * batch_size, test_outputs.shape[ 0 ]), : ]})[ :, 0 ]
