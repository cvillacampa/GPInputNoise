...
class NIMGPC(object):
    ...
    def __init__(self, kernels, inducing_points, n_classes, total_observed_x, input_var_ini, eps = 0.001, initialization_dict = None):
        ...
        if initialization_dict is not None:
            self.total_original_input_means = tf.Variable(initialization_dict['total_original_input_means'], dtype = tf.float32)
            self.total_original_input_log_vars = tf.Variable(initialization_dict['total_original_input_log_vars'], dtype = tf.float32)
            self.log_inputs_variances = tf.Variable(initialization_dict['log_inputs_variances'], dtype = tf.float32)
            self.Lraw = tf.Variable(initialization_dict['Lraw'], dtype = tf.float32)
            self.m = tf.Variable(initialization_dict['m'], dtype = tf.float32)


        if initialization_dict is None:
            self.total_original_input_means = tf.Variable(total_observed_x, dtype = tf.float32)
            self.total_original_input_log_vars = tf.Variable(tf.log(tf.ones(total_observed_x.shape) * 1e-5), dtype = tf.float32)

        self.train_indices = tf.placeholder(tf.int32, shape =  [None] , name = "train_indices")

        self.original_input_means = tf.gather(self.total_original_input_means, self.train_indices)
        self.original_input_log_vars = tf.gather(self.total_original_input_log_vars, self.train_indices)
        ...
        for e in range(epochs):
            ...
            for i in range(n_batches):
                ...
                avg_elbo += -1.0 * self.session.run(self.nelbo, feed_dict = \
                    {self.noisy_train_inputs: batch_x, self.train_outputs: batch_y, self.train_indices: original_indices[ train_indices ]})
        ...
        for i in range(n_batches):
            for c in range(self.n_classes):
                ...
                probs[ i * batch_size : np.minimum((i + 1) * batch_size, test_inputs.shape[ 0 ]), c : (c + 1) ] = \
                    self.session.run(self.test_probs, feed_dict = {self.noisy_test_inputs: test_inputs[ indices , : ], self.class_predict_test: np.array([ c ])})[:,None]
