...
def neural_network(noisy_inputs, input_dims, output_dims, n_units, n_layers = 1):
...
class NIMGPC_NN(object):
    ...
    def __init__(self, kernels, inducing_points, n_classes, total_observed_x, eps = 0.001, n_layers = 1, n_hiddens = 50):
        ...
        self.train_inputs_variances = tf.placeholder(tf.float32, shape = [ None, self.input_dim ], name = "train_inputs_variances")
        ...
        self.test_inputs_variances = tf.placeholder(tf.float32, shape = [ None, self.input_dim ], name = "test_inputs_variances")
        ...
        self.original_input_means, self.original_input_log_vars = neural_network(tf.concat([ self.noisy_train_inputs, \
            tf.cast(tf.one_hot(tf.reshape(self.train_outputs, (-1,)), self.n_classes, 1., 0.), tf.float32) ], 1), self.input_dims + self.n_classes, self.input_dims, n_hiddens, n_layers)
    ...
    def fit(self, training_points, training_targets, train_input_vars, optimizer, epochs, batch_size, test_points, test_targets, test_input_vars, file_results):
        ...
        for e in range(epochs):
            ...
            for i in range(n_batches):
                ...
                avg_elbo += -1.0 * self.session.run(self.nelbo, feed_dict = \
                    {self.noisy_train_inputs: batch_x, self.train_outputs: batch_y, self.train_inputs_variances: train_input_vars[ train_indices, : ]})
        ...
        ini_time = self.write_results(test_points, test_targets, test_input_vars, file_results, ini_time, test_targets.shape[ 0 ] / 10)
            ...
            for c in range(self.n_classes):
                ...
                probs[ i * batch_size : np.minimum((i + 1) * batch_size, test_inputs.shape[ 0 ]), c : (c + 1) ] = \
                    self.session.run(self.test_probs, feed_dict = {self.noisy_test_inputs: test_inputs[ indices , : ], \
		                             self.test_inputs_variances: test_input_vars[ indices, : ], self.class_predict_test: np.array([ c ])})[:,None]
